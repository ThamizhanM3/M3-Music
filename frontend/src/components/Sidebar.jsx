import { Home, Search, Library, PlusSquare, Heart, Users, LayoutDashboard, Music, Settings, LogOut, ArrowLeftRight, MoreVertical, Compass } from 'lucide-react';
  ...
  <NavLink 
    ...
    className={({ isActive }) => 
      `flex items-center gap-4 px-4 py-3 text-sm font-semibold rounded-md transition-all duration-200 ${
        isActive ? 'text-white bg-white/10 shadow-[inset_0_0_10px_rgba(255,255,255,0.05)]' : 'text-zinc-400 hover:text-white hover:bg-white/5'
      }`
    }
  >
...
const Sidebar = ({ isAdmin = false }) => {
  ...
  const handleCreatePlaylist = async () => {
    const playlist = await createPlaylist({ title: `My Playlist #${playlists.length + 1}` });
    if (playlist) {
  ...
  return (
    ...
    >
      <div className="px-6 mb-8 flex items-center gap-3">
        <div className="w-9 h-9 bg-primary rounded-lg flex items-center justify-center font-bold text-black shadow-[0_0_20px_var(--primary-glow)]">
          M3
        </div>
        <span className="text-xl font-bold tracking-tight neon-text">Music {isAdmin && <span className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded ml-1 text-zinc-400">ADMIN</span>}</span>
      </div>
      ...
      <nav className="flex flex-col gap-1 px-3 mb-6">
        {!isAdmin ? (
          <>
            ...
            {user?.role === 'admin' && (
              <NavLink to="/admin" className="flex items-center gap-4 px-4 py-3 text-sm font-semibold text-primary hover:bg-primary/10 rounded-md transition-all mt-4 border border-primary/20">
          ...
          <>
            ...
            <SidebarItem to="/admin/songs" icon={Music} label="Manage Songs" />
            <SidebarItem to="/admin/users" icon={Users} label="Manage Users" />
            <SidebarItem to="/admin/upload" icon={PlusSquare} label="Upload Music" />
            <NavLink to="/" className="flex items-center gap-4 px-4 py-3 text-sm font-semibold text-zinc-400 hover:text-white hover:bg-white/5 rounded-md transition-all mt-4 border border-white/5">
      ...
      {!isAdmin && (
        <div className="flex-1 flex flex-col min-h-0">
          <div className="px-6 mb-4">
            <h3 className="text-[10px] font-bold text-zinc-500 tracking-[0.2em] uppercase mb-4">Playlists</h3>
            <div className="flex flex-col gap-3">
              ...
              <SidebarItem to="/collection/tracks" icon={Heart} label="Liked Songs" />
            </div>
          </div>
              ...
              {playlists.map((playlist) => (
                ...
                >
                  ...
                  <div className="flex-1 truncate">
                    ...
                    <div className="text-[10px] text-zinc-500 truncate">Playlist • {playlist.totalSongs || 0} songs</div>
                  </div>
                </NavLink>
              ))}
            </div>
